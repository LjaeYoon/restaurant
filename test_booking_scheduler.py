...
MONDAY = datetime.strptime("2024/06/03 17:00", "%Y/%m/%d %H:%M")
...
def test_시간대별_인원제한이_있다_같은_시간대가_다르면_Capacity_차있어도_스케줄_추가_성공(booking_scheduler, customer):
    # arrange
    schedule = Schedule(ON_THE_HOUR, CAPACITY_PER_HOUR, customer)
    booking_scheduler.add_schedule(schedule)
    # act
...
def test_예약완료시_SMS는_무조건_발송됨(booking_scheduler, booking_scheduler_with_sms_mock, customer):
    # arrange
...
def test_현재날짜가_일요일인_경우_예약불가_예외처리(mocker, customer):
    # arrange
    mock_get_now = mocker.patch("booking_scheduler.BookingScheduler.get_now", return_value=datetime.strptime("2024/06/03 17:00", "%Y/%m/%d %H:%M"))
    booking_scheduler = BookingScheduler(CAPACITY_PER_HOUR)
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, customer)
...
def test_현재날짜가_일요일이_아닌경우_예약가능(mocker, customer):
    ...
    mock_get_now = mocker.patch("booking_scheduler.BookingScheduler.get_now",
                                return_value=datetime.strptime("2025/08/04 17:00", "%Y/%m/%d %H:%M"))
    booking_scheduler = BookingScheduler(CAPACITY_PER_HOUR)
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, customer)
